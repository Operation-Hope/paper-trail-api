"""Core Voteview CSV to Parquet conversion logic with streaming."""
...
def convert_voteview_file(
    ...
    *,
    ...
    """
    Convert a single Voteview CSV file to Parquet format using streaming.

    Args:
        source_path: Path to input CSV file
        output_path: Path for output .parquet file
        file_type: Type of Voteview file (members, rollcalls, votes)
    ...
    Raises:
        VoteviewConversionError: If conversion or validation fails
    """
    ...
    if validate:
        ...
        if config.sum_column:
            print(
                f"    Checksums: PASS ({config.sum_column} sum: "
                f"{validation_result.sum_column_actual:,.2f})"
            )
        else:
...
def _stream_csv_to_parquet(
    ...
    try:
        ...
        for batch in reader:
            ...
            # Progress every 10 batches or every 1M rows for large files
            if batch_num % 10 == 0 or stats.row_count % 1_000_000 == 0:
